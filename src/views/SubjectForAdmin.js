...
function SubjectForAdmin({subject, ...props}) {
  ...
  const getTeacherName = async () => {
    //setIsLoading(true);
    await axios.get(api + '/user/' + subject.teacherID, 
    {
      headers: {
  ...
  const getSubjectInfo = async () => {
    ...
    await axios.get(api + '/subject/' + subject.subjectID, 
    {
      headers: {
        Authorization: getToken()
      }
  ...
  const handleUpdate = e => {
    ...
    axios.put(api + '/subject/update/' + subject.subjectID, state,
    {
      headers: {
        Authorization: getToken()
      }
  ...
  const handleArchive = async ()=>{
    //axios.delete('http://localhost:5000/api/v1/subject/delete/' + subject.subjectID, 
    axios.get(api + '/subject/archive/' + subject.subjectID,
    {
      headers: {
        Authorization: getToken(),
      }
